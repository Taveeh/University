...
class Settings:
    ...
    def _setFiles(self):
        if self._type == 'inmemory':
            ...
            self.files.append(self._config.get('Settings', 'students').replace('"', ''))
            self.files.append(self._config.get('Settings', 'assignments').replace('"', ''))
            self.files.append(self._config.get('Settings', 'grades').replace('"', ''))
        else:
            self.files.append(self._config.get('Settings', 'students').replace('"', ''))
            self.files.append(self._config.get('Settings', 'assignments').replace('"', ''))
            self.files.append(self._config.get('Settings', 'grades').replace('"', ''))
    ...
    def main(self):
        if self._type == 'inmemory':
            ...
            StudentRepo = StudentSqlRepo(self._config.get('Settings', 'database').replace('"', ''), self.files[0])
            AssignmentRepo = AssignmentSqlRepo(self._config.get('Settings', 'database').replace('"', ''), self.files[1])
            GradesRepo = GradeSqlRepo(self._config.get('Settings', 'database').replace('"', ''), self.files[2])
        else:
            print('Invalid')
            return
        u = Undo()

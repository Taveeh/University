...
class OrderedGraphCost:
    ...
    def bellmanFord(self, start, maxLen):
        '''
        bellmanFord Function that applies the bellman ford algorithm on the graph
        ...
        Returns:
            list -- all the possible distances with length < maxLen
        '''
        initMap = {start: 0}
        dist = [initMap]
        for k in range(1, maxLen + 1):
            ...
            currentMap = {}
            for y in prevMap:
                for x in self.outboundEdge(y):
                    if x not in currentMap or currentMap[x] > prevMap[y] + self.getCost(y, x):
                        currentMap[x] = prevMap[y] + self.getCost(y, x)
            dist.append(currentMap)
        return dist

    def getMinCostWalk(self, dist, start, end, length):
        '''
        getMinCostWalk Function that gets the minimum cost walk between 2 nodes with given length

        Arguments:
            dist {list} -- distance list
            start {int} -- start vertex
            end {int} -- end vertex
            length {int} -- size of walk
        ...
        while currentLength > 0:
            walk.insert(0, currentVertex)
            for previousVertex in self.inboundEdge(currentVertex):
                if previousVertex in dist[currentLength - 1] and dist[currentLength - 1][previousVertex] + self.getCost(previousVertex, currentVertex) == dist[currentLength][currentVertex]:
